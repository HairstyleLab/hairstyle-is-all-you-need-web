...
async function main() {
    ...
    // URL 파라미터에서 파일 경로 읽기 (file 또는 url 파라미터, window.defaultURL)
    const fileParam = params.get("file") || params.get("url") || window.defaultURL || "train.splat";

    // 절대 URL인지 확인 (http:// 또는 https://로 시작)
    let url;
    if (fileParam.startsWith('http://') || fileParam.startsWith('https://')) {
        url = new URL(fileParam);
    } else {
        // 상대 경로면 현재 페이지 기준으로 해석
        url = new URL(fileParam, location.href);
    }
    ...
    window.addEventListener(
        ...
        (e) => {
            ...
            if (e.shiftKey) {
                ...
                // 휠 굴림에 따른 Z축 이동 (확대/축소)
                // e.deltaY가 양수면 뒤로, 음수면 앞으로
                let zoomSpeed = 0.005 * scale; // 속도 조절
                inv = translate4(inv, 0, 0, -e.deltaY * zoomSpeed);
            }
    ...
    canvas.addEventListener("mousemove", (e) => {
        ...
        if (down == 1) {
            ...
            // [수정 3] 상하 회전(dy) 제거 및 좌우 회전 제한 로직
            
            let d = 8; // 회전 축 거리
            inv = translate4(inv, 0, 0, d); // 피벗 포인트로 이동

            // 예상되는 다음 각도 계산
            let nextYaw = currentYaw + dx;

            // 45도(-YAW_LIMIT ~ YAW_LIMIT) 이내일 때만 회전 적용
            if (nextYaw > -YAW_LIMIT && nextYaw < YAW_LIMIT) {
                inv = rotate4(inv, dx, 0, 1, 0); // Y축(좌우) 회전만 적용
                currentYaw = nextYaw; // 현재 각도 업데이트
            }
            
            // inv = rotate4(inv, -dy, 1, 0, 0); // 상하 회전 코드 삭제 또는 주석 처리

            inv = translate4(inv, 0, 0, -d); // 다시 원위치
        ...
        // 마우스 우클릭 드래그 (이동 - 필요하다면 놔두고, 막으려면 이 블록도 수정 가능)
        // else if (down == 2) {
        //     let inv = invert4(viewMatrix);
        //     inv = translate4(
        //         inv,
